...
document.addEventListener('DOMContentLoaded', () => {
    // Create global TaskManager instance if it doesn't exist
    if (!window.taskManager) {
        window.taskManager = new TaskManager();
    }
    
    // Explicitly attach event listener to new task button
    const newTaskBtn = document.getElementById('newTaskBtn');
    if (newTaskBtn) {
        newTaskBtn.addEventListener('click', () => {
            if (window.taskManager && window.taskManager.currentProjectId) {
        ...
        console.log('New task button event listener attached');
    } else {
